...
def many_vowel_words(sentence)
    words = sentence.split(" ") # split should be done using a empty space, to separate words, not characters
    # debugger
...
def num_vowels(word)
    ...
    word.each_char do |char|
        count += 1 if "aeiou".include?(char) # the correct is "include", not "includes"
    end
    count
end
